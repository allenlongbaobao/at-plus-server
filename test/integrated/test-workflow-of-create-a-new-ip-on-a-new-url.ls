  ...
  describe '多人交互时，消息发送和接收正确', !->
    ...
    describe 'url为已有locatoin的alias时，消息次序正确', !->
      can '创建者收到response-create-a-new-ip-on-a-new-url，之前在此页面的用户收到push-location-updated', !(done)->
        ...
        creator.ip.emit 'request-create-a-new-ip-on-a-new-url', request-data, wait !(data)->
          debug "创建者收到创建成功消息"
    ...
    describe 'url为新的locatoin时，消息次序正确', !->
      can '创建者收到response-create-a-new-ip-on-a-new-url，之前在此页面的用户收到push-location-updated', !(done)->
        ...
        creator.locations.on 'ask-location-internality', !(data)->
          ...
          creator.locations.emit 'answer-location-internality', (is-internal: H.fake-figure-out-location-internality data.url, data.server-retrieved-html), !(data)->
...
debug-output-client-request-and-response-steps = !(locations-channel, ip-channel, done)->
  locations-channel.on 'ask-location-internality', !(data)->
    ...
    locations-channel.emit 'answer-location-internality', (is-internal: H.fake-figure-out-location-internality data.url, data.server-retrieved-html), !(data)->
      if data.errors and data.errors.length > 0
        debug '请求数据有误'
  ...
  ip-channel.emit 'request-create-a-new-ip-on-a-new-url', (utils.load-fixture 'request-create-a-new-ip-on-a-new-url'), !(data)->
